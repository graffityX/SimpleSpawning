...
public class setjoinmsg implements CommandExecutor{
	
	
	public static ArrayList<String> msgSetArray = new ArrayList<String>();
		...
		if(sender instanceof Player) {
			...
			if(args.length == 0) {
				if(player.hasPermission("sspawning.setjoinmsg") || player.hasPermission("sspawning.admin")) {
					...
					if(file.exists()) {
						player.sendMessage("§7Input the text you wanna use as join message! Use %player% for the player's name!");
						msgSetArray.add(player.getName());
				...
				player.sendMessage("§cToo many arguments! Please use /setjoinmsg!");
			}
